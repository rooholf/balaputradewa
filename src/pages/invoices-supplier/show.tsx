...
export const SupplierInvoiceShow: React.FC<IResourceComponentsProps> = () => {
    ...
    const dataSource = record?.vehicleOrders.map((item: IVehicleOrders, index: any) => ({
        ...
        product: record?.supplier.products.name,
        qty: item.qty,
        price: record?.supplierPrice?.price,
        total: parseCurrency(item.qty * record?.supplierPrice?.price as number),
    }));

    const subTotal = record?.vehicleOrders.reduce(
        (total: number, item: IVehicleOrders) => total + item.qty, 0)
        * record?.supplierPrice?.price
        || 0;
    ...
    return (
        ...
        >
            ...
            <Modal {...modalProps} width="60%" footer={null} style={{ top: 10 }}>
                <PdfLayout record={record as IOrder} />
            </Modal>
        </Show>
    );
};
